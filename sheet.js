...
// Assuming you have the following references to your input elements
const rowRangeFrom = document.getElementById('rowRangeFrom'); // Adjust ID as necessary
const rowRangeTo = document.getElementById('rowRangeTo');     // Adjust ID as necessary
const operationColumn = document.getElementById('operationColumn'); // Adjust ID as necessary
...
// Function to highlight the specified rows and column
function highlightCells() {
    const rowFrom = parseInt(rowRangeFrom.value) || 1; // Default to 1 if empty
    const rowTo = parseInt(rowRangeTo.value) || rowFrom; // Default to 'from' if 'to' is empty
    const operationColumnIndex = parseInt(operationColumn.value) - 1; // Convert to zero-based index

    // Clear previous highlights
    clearHighlights();

    // Highlight the selected rows for the operation column
    for (let i = rowFrom - 1; i <= rowTo - 1; i++) {
        if (data[i] && data[i][operationColumnIndex] !== undefined) {
            data[i][operationColumnIndex].classList.add('highlight'); // Add highlight class
        }
    }
}

// Add event listeners for dynamic highlighting
rowRangeFrom.addEventListener('input', highlightCells);
rowRangeTo.addEventListener('input', highlightCells);
operationColumn.addEventListener('input', highlightCells);
